...
bnds = tuple([(0.8, None) for i in range(x0.shape[0] * x0.shape[1])])
from scipy.optimize import NonlinearConstraint
# cons = NonlinearConstraint(lambda x: x[0] + x[1], ub=4, lb=3)
...
res = minimize(opt_fun, x0.flatten(), method='SLSQP', tol=1e-6, bounds=bnds, constraints=[cons])
print(res.x.reshape(-1, columns))
